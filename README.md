...
To build this project run (on the project's main folder)

```bash
  docker build -t riskapi .
```

To run this container on port 3000
```bash
  docker run -d -p 3000:80 riskapi
```

Now go to localhost:3000/ to check if the containeris running.

Then go to localhost:3000/docs to read the API documentation

There is an alternate documentation on localhost:3000/redoc


# 2. Main technical decisions

I used FastAPI (https://fastapi.tiangolo.com/) as the web framework because:

1. It has good performance
2. Standards-based
3. Automatic and beautiful API documentation

I also used Pydantic to create the Model type annotations since it is also used by FastAPI to form the request and response objects.


# 3. Relevant comments

The tests need to be refactored, since I opted for speed over quality.
This API is made to deliver high performance even in small servers. It has a very small memory footprint and can run on stateless environments.
